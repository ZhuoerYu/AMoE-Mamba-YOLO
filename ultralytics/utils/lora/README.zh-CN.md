...
- [`__init__.py`](ultralytics/utils/lora/__init__.py)：公共入口，负责兼容旧导入路径
- [`api.py`](ultralytics/utils/lora/api.py)：LoRA 主流程，包含配置解析、后端选择、目标层识别、`apply_lora()`
- [`config.py`](ultralytics/utils/lora/config.py)：`LoRAConfig` 与训练参数映射
- [`fallback.py`](ultralytics/utils/lora/fallback.py)：手写 fallback adapter、包装类、目标过滤逻辑
- [`io.py`](ultralytics/utils/lora/io.py)：adapter 保存、加载、可训练重载、权重合并
- [`training.py`](ultralytics/utils/lora/training.py)：训练策略、参数统计、LoRA 训练期辅助逻辑
...
在 [`ultralytics/engine/trainer.py`](ultralytics/engine/trainer.py) 中，`Trainer._setup_train()` 会先解析 LoRA 运行时参数，然后执行：
...
多数情况下，用户不需要直接操作底层 PEFT wrapper。仓库在 [`ultralytics/engine/model.py`](ultralytics/engine/model.py) 中提供了更符合 Ultralytics 风格的高层接口：
...
旧的单文件 `ultralytics/utils/lora.py` 已经删除，替换成当前这个目录包。但对外导入面没有丢，仍然可以通过 [`__init__.py`](ultralytics/utils/lora/__init__.py) 继续使用 `ultralytics.utils.lora` 入口。
