...
- [`__init__.py`](ultralytics/utils/lora/__init__.py): public entrypoint and backward-compatible re-exports
- [`api.py`](ultralytics/utils/lora/api.py): main orchestration for config resolution, backend selection, target detection, and `apply_lora()`
- [`config.py`](ultralytics/utils/lora/config.py): `LoRAConfig` and config normalization from trainer args
- [`fallback.py`](ultralytics/utils/lora/fallback.py): manual fallback adapters, wrappers, and target filtering helpers
- [`io.py`](ultralytics/utils/lora/io.py): save, load, reload-as-trainable, and merge logic
- [`training.py`](ultralytics/utils/lora/training.py): training strategies and parameter statistics
...
Inside [`ultralytics/engine/trainer.py`](ultralytics/engine/trainer.py), `Trainer._setup_train()` resolves LoRA-specific runtime values and then calls:
...
The low-level PEFT wrapper is not exposed directly to most users. Instead, [`ultralytics/engine/model.py`](ultralytics/engine/model.py) provides framework-native methods:
...
The old monolithic `ultralytics/utils/lora.py` was removed and replaced by this package. The public import surface is preserved through [`__init__.py`](ultralytics/utils/lora/__init__.py), so callers can still import from `ultralytics.utils.lora`.
...
- [`examples/lora_examples/README.md`](examples/lora_examples/README.md)
